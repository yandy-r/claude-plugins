...
| Lifecycle       | Fire, wait, read result, done              | `create an agent group` → spawn → monitor → `send follow-up instructions(shutdown)` → `close the agent group` |
...
- Do **not** call `record the task`, `the task tracker`, or `send follow-up instructions` for standalone sub-agents —
...
- **Do NOT** keep the turn alive with `sleep`, `echo`, or any other busywork "waiting" for
  ...
  Codex versions, a live turn blocks completion-notification delivery, so busywork
...
- **Do NOT** rely on "finished" notifications as the primary way to retrieve standalone
  ...
  read time.
- **Do NOT** call `send follow-up instructions` or `the task tracker` for standalone sub-agents. No shared task
