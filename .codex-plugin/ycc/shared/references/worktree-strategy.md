...
- **Codex**: prefer `Agent(isolation: "worktree")` with the pre-created child
...
| ----------- | ------------------------------------------------------ | ----------------------- |
| Codex | `Agent(isolation: "worktree")` + `WorktreeCreate` hook | Bash `git worktree add` |
...
## 7. Codex `WorktreeCreate` Hook Integration
...
`<repo>/.codex/worktrees/`. The `WorktreeCreate` hook registered in
