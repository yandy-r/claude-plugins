...
description: Plan and apply fixes for findings from a code-review artifact produced
  ...
  Pass `--parallel` to fan out independent fixes across parallel review-fixer agents.
  Pass `--severity <CRITICAL|HIGH|MEDIUM|LOW>` to change the minimum severity threshold
  (default HIGH). Pass `--dry-run` to preview the fix plan without applying changes.
  Use when the user asks to "fix review findings", "apply review fixes", "review-fix
  PR 42", "fix the code review", or says "/review-fix". Adapted from PRPs-agentic-eng
  by Wirasm.
---
