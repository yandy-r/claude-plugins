...
description: This skill should be used when the user asks to "initialize workspace",
  "set up Codex CLI environment", "configure agents and MCPs for project", "init workspace",
  "set up project with Claude tools", or mentions workspace initialization. Analyzes
  project type and presents available agents/MCPs for selection.
---

# Codex CLI Workspace Initialization

Initialize this workspace with the optimal Codex CLI configuration based on project analysis.
...
Initialize this workspace with the optimal Codex CLI configuration.
...
Read these files if they exist: `README.md`, `AGENTS.md`, `package.json`, `go.mod`, `requirements.txt` / `pyproject.toml`, `Cargo.toml`, `*.tf` files.
...
For selected agents: create `.claude/agents/` directory if needed and copy selected agent files from `${HOME}/.codex/agents/` to the project.

### Phase 6: Generate Summary Report

Output a summary using the template at `~/.codex/plugins/ycc/skills/init/templates/workspace-report.md` with project analysis, configured MCPs/agents tables, and next steps.
