...
This checklist is the artifact of record for a `bundle-release` run. It documents
...
- [ ] Version fields are in parity: `ycc/.codex-plugin/plugin.json` and `.codex-plugin/marketplace.json` agree before the bump.
...
- [ ] `ycc/.codex-plugin/plugin.json` — `version` field updated.
- [ ] `.codex-plugin/marketplace.json` — both `metadata.version` and `plugins[0].version` updated.
- [ ] Diff reviewed with `git diff` to confirm only version strings changed.

If this step fails: restore both files with `git checkout -- ycc/.codex-plugin/plugin.json .codex-plugin/marketplace.json` and investigate.
...
```
git add ycc/.codex-plugin/plugin.json \
...
1. Restore version files:
   ```
   git checkout -- ycc/.codex-plugin/plugin.json .codex-plugin/marketplace.json
   ```
...
See also: `references/version-policy.md`, `references/release-notes-template.md`, `AGENTS.md` (Testing Changes).
