...
Run `~/.codex/plugins/ycc/skills/bundle-release/scripts/preflight.sh`. On non-zero
...
Read `~/.codex/plugins/ycc/skills/bundle-release/references/version-policy.md`.
...
- Files that will be modified (`ycc/.codex-plugin/plugin.json`,
...
Run `~/.codex/plugins/ycc/skills/bundle-release/scripts/bump-version.sh <new-version>`.

This script updates both hand-edited JSON files atomically. After it exits, surface the
diff (`git diff ycc/.codex-plugin/plugin.json .codex-plugin/marketplace.json`) for
...
```
git checkout -- ycc/.codex-plugin/plugin.json .codex-plugin/marketplace.json .opencode-plugin/ && ./scripts/sync.sh
```
...
```
~/.codex/plugins/ycc/skills/bundle-release/scripts/draft-notes.sh <new-version>
```
