...
description: Execute a PRP plan file with continuous validation loops. Detects package
  ...
  per-batch shutdown via send follow-up instructions. Pass `--worktree` to force git
  worktree isolation per parallel task even when the plan has no worktree annotations
  (plans produced with `--worktree` are auto-detected and need no flag).
---
...
2. **Create child worktrees (when `WORKTREE_ACTIVE=true`, parallel tasks only)** — Before spawning agents, create a child worktree for each parallel task in the batch:
   ...
   CHILD_PATH=$(bash "~/.codex/plugins/ycc/shared/scripts/setup-worktree.sh" \
...
3. **Dispatch implementor agents in parallel** — Use a **SINGLE message** with **MULTIPLE `Agent` tool calls**, one per task in the batch. Each call:
   ...
   - **When `WORKTREE_ACTIVE=true` and the task is parallel**: append `Working directory: <child-path>` to the agent prompt. On Codex, also pass `isolation: "worktree"` pointing at the pre-created child path. The agent must treat that path as its repo root for all Read / Write / Edit / Bash calls.
...
3. **Spawn batch teammates** — Single message, multiple `Agent` tool calls, one per
   ...
   - **When `WORKTREE_ACTIVE=true` and task is parallel**: append
     `Working directory: <child-path>` to the prompt. On Codex, also pass
...
4. **Wait for batch completion via `the task tracker`** — poll until all tasks in this batch
