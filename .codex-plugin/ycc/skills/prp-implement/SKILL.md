...
- `WT_FEATURE_SLUG` — the segment after `<repo>-` in the parent path (e.g. `add-widget` from `<repo-root>/.codex/worktrees/my-repo-add-widget/`)
...
- `WT_PARENT_PATH` = `<repo-root>/.codex/worktrees/${WT_REPO_NAME}-${WT_FEATURE_SLUG}/`
...
This creates `<repo-root>/.codex/worktrees/${WT_REPO_NAME}-${WT_FEATURE_SLUG}/` on branch `feat/${WT_FEATURE_SLUG}`, branching from the current HEAD. The call is idempotent — if the parent already exists on the expected branch it echoes the path and exits 0.
...
> Run `git worktree remove <repo-root>/.codex/worktrees/<repo>-<feature>/` after merging and pushing to clean up the parent worktree.
