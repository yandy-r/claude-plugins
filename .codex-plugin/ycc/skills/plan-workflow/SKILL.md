...
- **--worktree**: Emit worktree annotations in the generated `parallel-plan.md` (Phase 8) — a top-level `## Worktree Setup` block and per-parallel-task `**Worktree**:` fields. Consumed by `$implement-plan --worktree` (or auto-detected). Follows `ycc/skills/_shared/references/worktree-strategy.md`. Combines freely with all other flags. No effect when `--research-only` is passed (no plan file is generated). Honored with `--plan-only`.
...
```bash
~/.codex/plugins/ycc/skills/plan-workflow/scripts/check-state.sh [feature-name]
```
...
```bash
cat ~/.codex/plugins/ycc/skills/plan-workflow/templates/checkpoint-messages.md
```
...
```bash
cat ~/.codex/plugins/ycc/skills/plan-workflow/templates/research-agents.md
```
...
All 4 `record the task` entries were registered up front in Step 9 — do not re-create them here.
...
After spawning, use `the task tracker` to confirm all 4 tasks are `completed` before proceeding to Step 11.
...
- **Path B (`--team`)**: check via `the task tracker` that all 4 research tasks are `completed`.
...
```bash
cat ~/.codex/plugins/ycc/skills/plan-workflow/templates/shared-structure.md
```
...
```bash
~/.codex/plugins/ycc/skills/plan-workflow/scripts/validate-shared.sh "${feature_dir}/shared.md"
```
...
- If `AGENT_TEAM_MODE=true`, clean up team (`close the agent group`). In standalone mode there is no team to tear down.
...
```bash
cat ~/.codex/plugins/ycc/skills/plan-workflow/templates/planning-agents.md
```
...
- **Path B (`--team`)**: check via `the task tracker` that all 3 analysis tasks are `completed`.
...
```bash
cat ~/.codex/plugins/ycc/skills/plan-workflow/templates/plan-structure.md
```
...
`~/.codex/plugins/ycc/shared/references/worktree-strategy.md` §2:
...
> following `~/.codex/plugins/ycc/shared/references/worktree-strategy.md` §2.
...
`~/.codex/plugins/ycc/shared/references/agent-team-dispatch.md` §7 so the
...
```bash
~/.codex/plugins/ycc/skills/plan-workflow/scripts/validate-workflow-plan.sh "${feature_dir}/parallel-plan.md"
```
...
```bash
cat ~/.codex/plugins/ycc/skills/plan-workflow/templates/validation-agents.md
```
...
- **Path B (`--team`)**: review findings via `the task tracker` and teammate messages.
...
8. In Path B, the team MUST be cleaned up (close the agent group) before skill completion
