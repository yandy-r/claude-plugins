...
~/.codex/plugins/ycc/skills/shared-context/scripts/validate-research-artifacts.sh "${feature_dir}" --optimized
```
...
- **Path B (`--team`)**: check via `the task tracker` that all 3 (standard) or 5 (optimized) analysis tasks are `completed`.
...
~/.codex/plugins/ycc/skills/parallel-plan/scripts/validate-analysis-artifacts.sh "${feature_dir}" --optimized
```
...
# Optimized mode:
~/.codex/plugins/ycc/skills/parallel-plan/scripts/persist-or-fail.sh "${feature_dir}" --optimized
```
...
`~/.codex/plugins/ycc/shared/references/agent-team-dispatch.md` §7 for
