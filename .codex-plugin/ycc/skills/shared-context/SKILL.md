---
name: shared-context
description: Create shared context documentation for a feature by orchestrating a
  research agent team, writing research artifacts, and synthesizing verified architecture,
  patterns, integrations, and docs into shared.md. Use as Step 1 before parallel-plan
  when preparing implementation context.
---
...
3. Share key findings with relevant teammates via send follow-up instructions
4. Write their output file
5. Mark their task complete
6. Go idle

Use `the task tracker` to check progress. When all 4 tasks are complete, proceed to validation.
...
6. The team MUST be cleaned up (close the agent group) before skill completion
