...
description: Create stunning, animation-rich, zero-dependency HTML presentations from
  scratch or by converting PowerPoint files. Enforces viewport-fit (one slide = one
  viewport, no scrolling), distinctive design presets, and a discovery-by-preview
  workflow. Use when the user asks to build a presentation/talk/pitch deck, convert
  a .ppt or .pptx to HTML, improve an existing HTML deck, asks about slide layout,
  slide animations, keyboard/touch navigation, viewport fitting with clamp() and 100dvh,
  presentation themes/style presets, or python-pptx for PPTX extraction.
---
...
Before generating, read `~/.codex/plugins/ycc/skills/frontend-slides/references/style-presets.md` for the viewport-safe CSS base, density limits, preset catalog, and CSS gotchas.
...
Use the preset guide in `~/.codex/plugins/ycc/skills/frontend-slides/references/style-presets.md` when mapping mood to style.
...
- the viewport-safe CSS base from the style-presets reference
...
Use the density limits and mandatory CSS block in the style-presets reference.
...
If browser automation is available (e.g. Playwright), use it to verify no slide overflows and that keyboard navigation works.
...
- invalid negated CSS functions like `-clamp(...)` (use `calc(-1 * clamp(...))` instead)
