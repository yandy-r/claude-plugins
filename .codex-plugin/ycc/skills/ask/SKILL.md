...
Spawn the `codebase-advisor` custom agent. This is the plugin's read-only agent — it has no write tools and cannot modify files.

**CRITICAL: Always use `codebase-advisor`. Never use `codebase-research-analyst`, `Explore`, or any other agent type.** Other agent types have write tools (Bash, Write, the task tracker) and will attempt to create files and directories.
