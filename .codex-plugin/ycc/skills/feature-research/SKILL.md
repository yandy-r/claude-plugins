...
description: Run the full 7-agent research pipeline for a new feature — deploys api-researcher,
  ...
  research track that outputs to docs/plans/, NOT docs/prps/. Use when the user asks
  to "research a feature", "run feature research", "deep-dive a feature before planning",
...
- Uses create an agent group/close the agent group for agent coordination
...
```
create an agent group: name="fr-[feature-name]", description="Feature research team for [feature-name]"
```
...
**CRITICAL**: Spawn all 7 teammates in a **SINGLE message** with **MULTIPLE parallel agent runs**.
...
Wait for all 7 teammates to complete their tasks. Use `the task tracker` to check progress.
