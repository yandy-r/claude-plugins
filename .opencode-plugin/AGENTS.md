<!-- Generated from ycc/settings/rules/CLAUDE.md by scripts/generate_opencode_plugin.py — do not edit by hand. -->

# Agent Rules (User-Global)

Generic agent-runtime guidance that applies to any project. Project-specific rules
in the current repo's `AGENTS.md` / `AGENTS.md` override these.

This file is installed by `install.sh --settings` and symlinked into each supported
runtime's user-global config directory (opencode, Cursor, Codex, opencode).

## Precedence

1. System, developer, and explicit user instructions for the task.
2. The current project's `AGENTS.md` / `AGENTS.md` as repo policy.
...
## opencode Plugin Install Model

A plugin installed from a **`directory`-source marketplace** (`source: directory`
in `~/.config/opencode/plugins/known_marketplaces.json`) is loaded **in place** from its
source repo — opencode never copies it into `~/.config/opencode/plugins/cache/`. So a
missing `~/.config/opencode/plugins/cache/<marketplace>/<plugin>/<version>/` is **expected,
