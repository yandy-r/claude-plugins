---
description: 'Bootstrap a best-practices lint/format environment — installs a self-contained
  scripts/style.sh bundle, per-language tool configs, runnable aliases (package.json/Makefile/justfile),
  README docs, and optionally CI + pre-commit hooks. Usage: [--dry-run] [--force]
  [--yes] [--sync] [--copy] [--ci] [--hooks] [--no-aliases] [--no-docs] [--target=<dir>]
  [--profile=<lang>] [--rust] [--ts] [--python] [--go] [--docs] [--shell] [--all]'
---
...
| Command                                                  | Effect                                                                         |
| -------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `/formatters`                                        | Auto-detect stacks and bootstrap the current project.                          |
| `/formatters --dry-run`                              | Preview every planned change without writing any files.                        |
| `/formatters --sync`                                 | Re-run after a prior install; prune stale managed files.                       |
| `/formatters --ci --hooks`                           | Also emit `.github/workflows/lint.yml` and wire a pre-commit hook.             |
| `/formatters --no-aliases --no-docs`                 | Install scripts and configs only; skip package.json/Makefile and README edits. |
| `/formatters --target=~/projects/app --profile=rust` | Bootstrap a different directory with a forced Rust profile.                    |
| `/formatters --all --force --yes`                    | Non-interactive full install regardless of detection (CI-friendly).            |
...
See `~/.config/opencode/skills/formatters/references/flag-reference.md` for the full matrix and precedence rules, and `best-practices.md` for per-language tool rationale.
