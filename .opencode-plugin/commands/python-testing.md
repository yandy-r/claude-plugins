...
description: 'Python testing patterns using pytest — TDD methodology, fixtures (function/module/session
  ...
  for a Python project. Usage: None'
---

# Python Testing Command

Provide Python testing patterns and TDD guidance using `pytest` as the primary test runner.

**Load and follow the `python-testing` skill**, passing through `$ARGUMENTS`.

The skill covers:

- Fixtures with function/module/session scopes
- `@pytest.mark.parametrize` and markers
- Mocking with `unittest.mock` (`@patch`, `Mock`, `MagicMock`, `autospec`)
- Async tests with `pytest-asyncio`
- `tmp_path` and `conftest.py` organization
- Coverage targets with `pytest --cov`
- TDD workflow for Python projects
