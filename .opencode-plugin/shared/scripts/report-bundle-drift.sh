...
resolve_repo_root_from() {
  ...
  while [[ "$candidate" != "/" ]]; do
    if [[ -f "$candidate/.opencode-plugin/marketplace.json" ]]; then
      REPO_ROOT="$candidate"
      return 0
    fi
    candidate="$(dirname "$candidate")"
  done
  if [[ -f "/.opencode-plugin/marketplace.json" ]]; then
    REPO_ROOT="/"
    return 0
  fi
  return 1
...
if [[ -z "$REPO_ROOT" ]]; then
  echo "report-bundle-drift.sh: cannot locate repo root (no .opencode-plugin/marketplace.json found from repo-root input/PWD/SCRIPT_DIR search)" >&2
  exit 2
fi
