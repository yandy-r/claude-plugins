...
- **opencode**: prefer `Agent(isolation: "worktree")` with the pre-created child
...
| ----------- | ------------------------------------------------------ | ----------------------- |
| opencode | `Agent(isolation: "worktree")` + `WorktreeCreate` hook | Bash `git worktree add` |
...
## 7. opencode `WorktreeCreate` Hook Integration
...
`<repo>/.config/opencode/worktrees/`. The `WorktreeCreate` hook registered in
