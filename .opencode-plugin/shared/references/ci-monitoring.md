...
- **PR mode** — `~/.config/opencode/shared/scripts/ci-monitor.sh`
  watches checks on a PR head branch (used by `git-workflow`, `prp-pr`,
  `pr-autofix`).
- **Release mode** — `~/.config/opencode/shared/scripts/release-ci-monitor.sh`
...
scripts via `~/.config/opencode/shared/scripts/lib/ci-classify.sh`.
...
- **Audit log**: every script invocation appends a JSONL record to
  `~/.config/opencode/session-data/ci-watch/<pr>-<utc-timestamp>.log`. The path is
...
2. **Initialize audit log path**: compute
   `~/.config/opencode/session-data/ci-watch/<pr>-<utc-iso-timestamp>.log`. Create the
...
4. **Branch on stdout `RESULT=...`**:
   ...
   - **`handoff`** — read the following fields from stdout:
     ...
     `~/.config/opencode/skills/git-workflow/scripts/validate-commit.sh`.
...
| Audit log path         | `~/.config/opencode/session-data/ci-watch/`    | `~/.config/opencode/session-data/release-ci-watch/`                                                                                         |
...
- `~/.config/opencode/shared/scripts/ci-monitor.sh` — PR-mode execution script
- `~/.config/opencode/shared/scripts/release-ci-monitor.sh` — release-mode execution script
- `~/.config/opencode/shared/scripts/lib/ci-classify.sh` — shared failure classification + signature lib
- `~/.config/opencode/skills/git-workflow/SKILL.md` — Phase 6 (PR CI monitoring loop)
- `~/.config/opencode/skills/prp-pr/SKILL.md` — Phase 7 (PR CI monitoring loop)
- `~/.config/opencode/skills/pr-autofix/SKILL.md` — Phase 7 (PR CI monitoring loop)
- `~/.config/opencode/skills/releaser/SKILL.md` — Phase 8.5 (release CI monitoring loop)
