...
#
# Updates:
#   - ycc/.opencode-plugin/plugin.json (.version)
#   - .opencode-plugin/marketplace.json (.metadata.version and .plugins[0].version)
set -euo pipefail
...
print(f"ycc/.opencode-plugin/plugin.json:       {old_plugin} -> {new_version}")
print(f".opencode-plugin/marketplace.json meta: {old_meta} -> {new_version}")
print(f".opencode-plugin/marketplace.json entry: {old_entry} -> {new_version}")
PY
