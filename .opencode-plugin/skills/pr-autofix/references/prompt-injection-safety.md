...
Always validate the final message via `~/.config/opencode/skills/git-workflow/scripts/validate-commit.sh` before pushing.
...
- **Fixer agent imagination.** If the fixer hallucinates a fix unrelated to the reviewer's comment, no sanitization will catch that. The defense is the agent's narrow scope discipline (see `.opencode-plugin/agents/pr-comment-fixer.md`) and the user's per-comment approval (when `--yes` is not set).
