...
description: Create detailed parallel implementation plans by running analysis and
  ...
  to standalone parallel sub-agents via the opencode `task` tool; pass `--team` (Claude
  Code only) to orchestrate the analysis and validation stages as teammates under
  a shared spawn coordinated subagents/the todo tracker with coordinated shutdown.
---
...
```bash
~/.config/opencode/skills/parallel-plan/scripts/check-prerequisites.sh [feature-name]
```
...
- With `--team` (`AGENT_TEAM_MODE=true`): create team `pp-[feature-name]`, register analysis tasks, spawn 3 analysis teammates under the team, validate, generate plan, register validation tasks, spawn 3 validation teammates, review, then `end the coordinated run`. Teammates share findings via `send follow-up instructions`.
...
```bash
cat ~/.config/opencode/skills/parallel-plan/templates/analysis-prompts.md
```
...
> WORKTREE MODE: In your `analysis-tasks.md` output, clearly label each task as **parallel** (can run concurrently with siblings) or **sequential** (must follow a predecessor). This labeling is used by the orchestrator for batch ordering. Follow `.opencode-plugin/skills/_shared/references/worktree-strategy.md` for the single-worktree contract.
...
All 3 `track the task` entries were registered up front in Step 6 — do not re-create them here.
...
**WORKTREE MODE (Path B)**: By default (`WORKTREE_MODE=true`), append the following directive to the `task-structurer` prompt (same directive as Path A above). Omit when `--no-worktree` was passed. Cross-reference `.opencode-plugin/skills/_shared/references/worktree-strategy.md` (single-worktree contract) and `agent-team-dispatch.md` §7 in the teammate prompt.
...
- **Path B (`--team`)**: use `the todo tracker` to check progress. Teammates share findings with each other via `send follow-up instructions`. If a teammate gets stuck, send them guidance via `send follow-up instructions`.
...
```bash
cat ~/.config/opencode/skills/parallel-plan/templates/plan-structure.md
```
...
section entirely. Follow `~/.config/opencode/shared/references/worktree-strategy.md`
...
```bash
cat ~/.config/opencode/skills/parallel-plan/templates/validation-prompts.md
```
...
After spawning, use `the todo tracker` to confirm all 3 tasks are `completed`.
...
```bash
~/.config/opencode/skills/parallel-plan/scripts/validate-parallel-plan.sh "${feature_dir}/parallel-plan.md"
```
...
7. In Path B, the team MUST be cleaned up (end the coordinated run) before skill completion
...
- **Choose dispatch mode from `$ARGUMENTS`** - default is standalone sub-agents via `Task`; `--team` switches to teammates under `spawn coordinated subagents`/`the todo tracker`; a single feature worktree section is included by default (`--no-worktree` suppresses it)
