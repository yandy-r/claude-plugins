...
```bash
cat ~/.config/opencode/skills/git-workflow/templates/commit-types.md
```
...
```bash
~/.config/opencode/skills/git-workflow/scripts/analyze-changes.sh
```
...
```bash
cat ~/.config/opencode/skills/git-workflow/templates/documentation-decision.md
```
...
```bash
~/.config/opencode/skills/git-workflow/scripts/validate-commit.sh "<commit-message>"
```
...
```bash
cat ~/.config/opencode/skills/git-workflow/templates/agent-prompts.md
```

### Step 15: Deploy Agents in Parallel

**CRITICAL**: Deploy all agents in a **SINGLE message** with **MULTIPLE parallel subagent invocations**.
...
```bash
~/.config/opencode/skills/git-workflow/scripts/validate-commit.sh "$(git log -1 --format=%s <commit-hash>)"
```

- If any commit message fails validation, amend it:

```bash
git rebase -x '~/.config/opencode/skills/git-workflow/scripts/validate-commit.sh "$(git log -1 --format=%s HEAD)" || echo "NEEDS FIX"' <merge-base>..HEAD
```
...
```bash
cat ~/.config/opencode/skills/git-workflow/templates/pr-template.md
```
...
```bash
~/.config/opencode/skills/git-workflow/scripts/create-pr.sh --analyze
```
...
```bash
~/.config/opencode/skills/git-workflow/scripts/validate-commit.sh "<pr-title>"
```
...
Read `~/.config/opencode/shared/references/ci-monitoring.md` to load the failure-classification table, termination policy, audit log schema, and loop protocol. Treat that file as authoritative — do not duplicate its content here.
...
Create directory `~/.config/opencode/session-data/ci-watch/` if absent. Compute log path `~/.config/opencode/session-data/ci-watch/<pr>-<utc-iso-timestamp>.log` and remember it for the loop.
...
See `~/.config/opencode/shared/references/ci-monitoring.md` for the full policy (classification, termination, schema).
...
- **Stop** — any `bail-*` (`bail-recurrence`, `bail-nonfixable`, `bail-pushes`,
  ...
  bail codes. See `~/.config/opencode/shared/references/ci-monitoring.md` for
