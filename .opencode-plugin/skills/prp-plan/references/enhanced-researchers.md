...
> **Contract**: Each of these 7 researchers feeds the `prp-plan` synthesizer in Phase 6. They DO NOT write any files. They return findings inline as structured rows that map directly to a section of the PRP plan template at `~/.config/opencode/skills/prp-plan/references/plan-template.md`. All 7 use `@prp-researcher`, distinguished only by `name=` and the role-specific prompt below.
