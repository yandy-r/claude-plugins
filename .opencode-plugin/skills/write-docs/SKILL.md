...
```bash
~/.config/opencode/skills/write-docs/scripts/audit-documentation.sh
```
...
Use **the todo tracker** to create a prioritized task list:
...
**CRITICAL**: Deploy all 5 agents in a **SINGLE message** with **MULTIPLE parallel subagent invocations**.
...
```bash
~/.config/opencode/skills/write-docs/scripts/generate-doc-index.sh
```
...
```bash
~/.config/opencode/skills/write-docs/scripts/verify-links.sh
```
