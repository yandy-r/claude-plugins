...
When worktree mode is active, wrap each severity batch with the 5-step lifecycle from `.opencode-plugin/skills/_shared/references/worktree-strategy.md`. The pattern is identical for Paths A, B, and C; only the agent dispatch mechanism inside Step 2 differs.
...
2. **Dispatch review-fixer agents** (Path-specific — see each path below). All agents receive:
   ...
   - On opencode, prefer `Agent(isolation: "worktree")` with the pre-created child path (the `WorktreeCreate` hook redirects correctly).
...
- When `WORKTREE_ACTIVE=true`: batches are severity-keyed. Dispatch teammates with `Working directory: <CHILD_PATH>` + `isolation: "worktree"` (opencode). Before each merge-children call, `send follow-up instructions(shutdown)` to all batch teammates. Then merge → advance to the next severity.
