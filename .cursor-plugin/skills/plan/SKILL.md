...
**Note**: `--parallel` on `/plan` shapes the _output_, not the research phase. For research fan-out on larger features, use `/prp-plan --parallel` (sub-agent fan-out) or `/prp-plan --agent-team` (Claude Code only; shared-task-list coordination).
...
- **`/prp-plan --agent-team`** — Team-coordinated research with shared TaskList for medium/large features that will produce an artifact file.
- **`/prp-implement --agent-team`** — Team-coordinated execution with shared TaskList across all batches. Best for implementation runs where you want coordinated inter-batch shutdown and a single shared task graph.
