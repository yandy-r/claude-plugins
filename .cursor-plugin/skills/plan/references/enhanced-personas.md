...
>
> Both enhanced personas use `subagent_type: "research-specialist"` distinguished by `name=`. They contribute slices that the synthesizer folds into the merged plan per `SKILL.md` §B.7 (Path B) or §C.5 (Path C). They do NOT write any files — they return findings inline.
