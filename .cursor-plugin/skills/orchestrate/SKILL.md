...
When `WORKTREE_MODE=true` and `DRY_RUN=true`: skip all script calls. Instead, compute the expected parent path as `<repo-root>/.cursor/worktrees/<repo>-<FEATURE_SLUG>/` and proceed to Phase 3 to include it in the dry-run output.
...
```
Worktree:   feature=<repo-root>/.cursor/worktrees/<repo>-<FEATURE_SLUG>/  (all subtasks)
```
...
See `${CURSOR_PLUGIN_ROOT}/skills/_shared/references/standalone-dispatch.md` for the full `Task`-vs-`Agent` dispatch contract and anti-patterns this path follows.
...
**2. Spawn ALL batch agents in a SINGLE message** using MULTIPLE `Task` tool calls. **No `team_name`, no `name`, no `TaskCreate`** — standalone sub-agent semantics. Each prompt must use the **Path A coordination block** from `agent-prompts.md` (standalone implementor — no inter-agent coordination).
