...
Consumed by `git-workflow` (Phase 6), `prp-pr` (Phase 7), `pr-autofix`
(Phase 7), and `releaser` (Phase 8.5) when the `--ci` flag is passed. This
...
- **PR mode** — `${CURSOR_PLUGIN_ROOT}/skills/_shared/scripts/ci-monitor.sh`
  watches checks on a PR head branch (used by `git-workflow`, `prp-pr`,
  `pr-autofix`).
- **Release mode** — `${CURSOR_PLUGIN_ROOT}/skills/_shared/scripts/release-ci-monitor.sh`
...
scripts via `${CURSOR_PLUGIN_ROOT}/skills/_shared/scripts/lib/ci-classify.sh`.
...
- **Audit log**: every script invocation appends a JSONL record to
  `~/.cursor/session-data/ci-watch/<pr>-<utc-timestamp>.log`. The path is
...
The SKILL.md files for `git-workflow` and `prp-pr` implement the
...
2. **Initialize audit log path**: compute
   `~/.cursor/session-data/ci-watch/<pr>-<utc-iso-timestamp>.log`. Create the
...
4. **Branch on stdout `RESULT=...`**:
   ...
   - **`handoff`** — read the following fields from stdout:
     ...
     `${CURSOR_PLUGIN_ROOT}/skills/git-workflow/scripts/validate-commit.sh`.
...
Used by `releaser` Phase 8.5 (`--ci`) **only** after a successful
...
| Audit log path         | `~/.cursor/session-data/ci-watch/`    | `~/.cursor/session-data/release-ci-watch/`                                                                                         |
| Fix-agent type         | `pr-comment-fixer`                | `release-fix-applier`                                                                                                          |
...
2. **Fallback: destructive re-cut (requires `--ci-recut=destructive`).** The
   ...
   ${CURSOR_PLUGIN_ROOT}/skills/releaser/scripts/publish-release.sh \
...
`releaser` Phase 8.5.0 inspects the workflow's `on:` block and refuses to
...
- **Never edit release artifacts**: `CHANGELOG.md`, files under `docs/releases/`,
  ...
  agent (`release-fix-applier`) refuses these paths.
...
- `${CURSOR_PLUGIN_ROOT}/skills/_shared/scripts/ci-monitor.sh` — PR-mode execution script
- `${CURSOR_PLUGIN_ROOT}/skills/_shared/scripts/release-ci-monitor.sh` — release-mode execution script
- `${CURSOR_PLUGIN_ROOT}/skills/_shared/scripts/lib/ci-classify.sh` — shared failure classification + signature lib
- `${CURSOR_PLUGIN_ROOT}/skills/git-workflow/SKILL.md` — Phase 6 (PR CI monitoring loop)
- `${CURSOR_PLUGIN_ROOT}/skills/prp-pr/SKILL.md` — Phase 7 (PR CI monitoring loop)
- `${CURSOR_PLUGIN_ROOT}/skills/pr-autofix/SKILL.md` — Phase 7 (PR CI monitoring loop)
- `${CURSOR_PLUGIN_ROOT}/skills/releaser/SKILL.md` — Phase 8.5 (release CI monitoring loop)
