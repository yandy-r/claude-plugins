...
- If `WORKTREE_ACTIVE=true` and the plan had no annotations (the default-on fallback), also set `WT_PARENT_PATH` = `<repo-root>/.cursor/worktrees/${WT_REPO_NAME}-${WT_FEATURE_SLUG}/`.
...
See `${CURSOR_PLUGIN_ROOT}/skills/_shared/references/standalone-dispatch.md` for the full standalone-dispatch contract (`Task` vs `Agent`+team, anti-patterns, backstop policy). Standalone batches dispatch via the blocking `Task` tool — never background, poll, or sleep waiting on a result; the sub-agent's report is already in hand when the `Task` call returns.
...
- If `WORKTREE_ACTIVE=true` and the run aborts mid-batch, the feature worktree at `<repo-root>/.cursor/worktrees/<repo>-<feature>/` survives and can be inspected or cleaned up manually.
...
> The parent worktree at `<repo-root>/.cursor/worktrees/[repo]-[feature]/` survives for inspection
> and PR creation. When you are done, run:
>
> git worktree remove <repo-root>/.cursor/worktrees/[repo]-[feature]/
