...
| --------------------------------- | ----------- | ---------------------------------- |
| `ycc/.cursor-plugin/plugin.json`  | hand-edited | `bundle-release` `bump-version.sh` |
| `.cursor-plugin/marketplace.json` | hand-edited | `bundle-release` `bump-version.sh` |
| `.codex-plugin/ycc/**`            | regenerated | `scripts/generate-codex-plugin.sh` |
| `.cursor-plugin/**`               | regenerated | `scripts/sync.sh --only cursor`    |

## Parity Rule

`ycc/.cursor-plugin/plugin.json:"version"` and both `"version"` fields inside
`.cursor-plugin/marketplace.json` (top-level `metadata.version` and
