...
allowed-tools:
  ...
  - Bash(git:*)
  - Bash(python3:*)
  - 'Bash(${CURSOR_PLUGIN_ROOT}/skills/bundle-release/scripts/*.sh:*)'
...
and Codex bundles, running the full validator sweep, and drafting release notes. It
...
Run `${CURSOR_PLUGIN_ROOT}/skills/bundle-release/scripts/preflight.sh`. On non-zero
...
- Version parity: `ycc/.cursor-plugin/plugin.json` and `.cursor-plugin/marketplace.json`
...
Read `${CURSOR_PLUGIN_ROOT}/skills/bundle-release/references/version-policy.md`.
...
- Files that will be modified (`ycc/.cursor-plugin/plugin.json`,
  `.cursor-plugin/marketplace.json`).
- Derived bundles that will be regenerated (`.cursor-plugin/`, `.codex-plugin/`).
...
Run `${CURSOR_PLUGIN_ROOT}/skills/bundle-release/scripts/bump-version.sh <new-version>`.

This script updates both hand-edited JSON files atomically. After it exits, surface the
diff (`git diff ycc/.cursor-plugin/plugin.json .cursor-plugin/marketplace.json`) for
...
```
git checkout -- ycc/.cursor-plugin/plugin.json .cursor-plugin/marketplace.json && ./scripts/sync.sh
```

Do not hand-edit `.cursor-plugin/` or `.codex-plugin/` — these are regenerated only.
...
```
${CURSOR_PLUGIN_ROOT}/skills/bundle-release/scripts/draft-notes.sh <new-version>
```
...
git add ycc/.cursor-plugin/plugin.json \
        .cursor-plugin/marketplace.json \
...
- Never auto-commits or pushes. The user reviews all changes and runs the emitted
  commands manually.
- Never hand-edits `.cursor-plugin/` or `.codex-plugin/` — these paths are fully
