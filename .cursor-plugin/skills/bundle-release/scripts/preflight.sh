...
# ycc/.cursor-plugin/plugin.json and .cursor-plugin/marketplace.json, and warns
...
Gate checks before a ycc release:
  ...
  2. current version parity across ycc/.cursor-plugin/plugin.json
...
PLUGIN_VERSION="$(read_json_value "${REPO_ROOT}/ycc/.cursor-plugin/plugin.json" '["version"]')"
MARKETPLACE_META_VERSION="$(read_json_value "${REPO_ROOT}/.cursor-plugin/marketplace.json" '["metadata"]["version"]')"
MARKETPLACE_PLUGIN_VERSION="$(read_json_value "${REPO_ROOT}/.cursor-plugin/marketplace.json" '["plugins"][0]["version"]')"
...
if ! [[ "${PLUGIN_VERSION}" == "${MARKETPLACE_META_VERSION}" && "${PLUGIN_VERSION}" == "${MARKETPLACE_PLUGIN_VERSION}" ]]; then
    ...
    echo "  ycc/.cursor-plugin/plugin.json .version           = ${PLUGIN_VERSION}" >&2
    echo "  .cursor-plugin/marketplace.json .metadata.version = ${MARKETPLACE_META_VERSION}" >&2
    echo "  .cursor-plugin/marketplace.json .plugins[0].version = ${MARKETPLACE_PLUGIN_VERSION}" >&2
    exit 1
fi
