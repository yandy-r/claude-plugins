...
section entirely. Follow `${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/worktree-strategy.md`
...
> **Plan-file handoff**: leave `parallel-plan.md` and `shared.md` in `docs/plans/<feature-slug>/` (main checkout). The implementor (`ycc:implement-plan`) will **move** them into the feature worktree once it's created — never copied. See `worktree-strategy.md` §7.
