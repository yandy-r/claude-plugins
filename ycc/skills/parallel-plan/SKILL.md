...
`${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/worktree-strategy.md` exactly for
...
- **Choose dispatch mode from `$ARGUMENTS`** - default is standalone sub-agents via `Task`; `--team` switches to teammates under `TeamCreate`/`TaskList`; `--worktree` emits worktree annotations in the plan
