...
Audit criteria used by the `ycc:releaser` agent in `--ci-config=audit` mode, and the
...
| `workflow_dispatch` is included for manual re-runs (REQUIRED if the project uses `/ycc:releaser --ci` non-destructively; the auto-fix loop re-triggers via `gh workflow run`) | high     |
...
The release-CI auto-fix loop driven by `/ycc:releaser --ci` needs a way to
