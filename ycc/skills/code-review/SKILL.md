...
- **`--worktree` + concurrent runs**: two `/ycc:code-review --worktree <N>` invocations on different PR numbers never collide (each gets its own `<repo>-pr-<N>/` directory). Two invocations on the SAME PR number share the same parent worktree (idempotent), which is by design.
