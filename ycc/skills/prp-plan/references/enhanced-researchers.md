# Enhanced Researcher Prompts (7-agent fan-out for ycc:prp-plan --enhanced)

> **Contract**: Each of these 7 researchers feeds the `ycc:prp-plan` synthesizer in Phase 6. They are dispatched standalone via the blocking `Task` tool (see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/standalone-dispatch.md`). Each researcher returns its findings inline as structured rows that map directly to a section of the PRP plan template at `${CLAUDE_PLUGIN_ROOT}/skills/prp-plan/references/plan-template.md`, **AND** additionally writes the same discovery table to a backstop file at `docs/prps/plans/.prp-research/<feature-slug>/<role-name>.md` using the `Write` tool, so the findings are never lost even if the inline return is interrupted or truncated. All 7 use `subagent_type: "ycc:prp-researcher"`, distinguished only by `name=` and the role-specific prompt below.
...
```
You are feeding the `ycc:prp-plan` synthesizer. Return findings inline as
...
- Do NOT reference SendMessage or inter-teammate coordination
```
