...
Use the Task tool with **`subagent_type: "ask:codebase-advisor"`**. This is the plugin's read-only agent — it has no write tools and cannot modify files.

**CRITICAL: Always use `subagent_type: "ask:codebase-advisor"`. Never use `codebase-research-analyst`, `Explore`, or any other agent type.** Other agent types have write tools (Bash, Write, TodoWrite) and will attempt to create files and directories.
...
- The advisor reads CLAUDE.md and project conventions to provide context-aware guidance
