...
description: >
  ...
  or starts with clarifying questions. Use when the user asks to "write a spec",
  "spec out a feature", "quick spec", "create a PRP spec", or says "/prp-spec".
...
- Run `/ycc:prp-prd` first if a full hypothesis-driven PRD is needed
```
...
6. **Do NOT act as a PRD generator**: This skill is not problem-first and does not run hypothesis questioning. For that, use `/ycc:prp-prd`.

---

## Integration

- **Upstream**: Accepts PRD files from `/ycc:prp-prd` or free-form descriptions. Can be invoked standalone.
...
- **Parallel track**: For heavyweight multi-agent research, use `/ycc:feature-research` instead (outputs to `docs/plans/`).
