...
`${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/ci-monitoring.md` to load the
...
**Step 4 — Initialize audit log:** Create `~/.claude/session-data/ci-watch/` if
absent. Compute log path `~/.claude/session-data/ci-watch/<pr>-<utc-iso-timestamp>.log`.
...
- `handoff` → Read `RUN_ID`, `WORKFLOW`, `JOB`, `CATEGORY`, `SIGNATURE`,
  ...
  `${CLAUDE_PLUGIN_ROOT}/skills/git-workflow/scripts/validate-commit.sh`. Commit
...
See `${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/ci-monitoring.md` for the
full policy.
