...
`ycc:pr-autofix` defends against this with two complementary sanitization passes:

1. **Inbound sanitization** — applied before a comment body is shown to the user or handed to a `ycc:pr-comment-fixer` agent.
...
When the user provides a free-form reason for skipping (via AskUserQuestion), pass it through the same filters as inbound sanitization before posting:
...
Always validate the final message via `${CLAUDE_PLUGIN_ROOT}/skills/git-workflow/scripts/validate-commit.sh` before pushing.
