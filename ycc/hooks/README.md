...
Hooks are event-driven automations that fire before or after Claude Code tool executions. They
enforce code quality, catch mistakes early, and automate repetitive checks.
...
- **PreToolUse** hooks run before the tool executes. They can **block** (exit code 2) or **warn**
...
Remove or comment out the hook entry in `hooks.json`. If installed as a plugin, override in your
...
export YCC_HOOK_PROFILE=standard

# Disable specific hook IDs (comma-separated)
export YCC_DISABLED_HOOKS="pre:bash:tmux-reminder,post:edit:typecheck"
```
...
Hook logic is implemented in Node.js scripts for cross-platform behavior on Windows, macOS, and
Linux. A small number of shell wrappers are retained for continuous-learning observer hooks; those
wrappers are profile-gated and have Windows-safe fallback behavior.
