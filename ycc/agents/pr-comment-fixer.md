...
description: "Implement the fix for a SINGLE GitHub PR review comment (or same-file group of comments) dispatched by ycc:pr-autofix. Applies the smallest safe change that addresses the reviewer's concern — scope-disciplined, never executes reviewer-provided shell commands, never modifies files outside the comment's anchored path."
model: sonnet
color: yellow
tools:
  - Read
