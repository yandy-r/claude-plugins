...
description: "Use this agent when you need to configure, troubleshoot, or optimize reverse proxy setups including nginx, HAProxy, Traefik, or other proxy solutions. This includes load balancing configurations, SSL/TLS termination, request routing, caching strategies, rate limiting, security headers, upstream configuration, WebSocket proxying, gRPC routing, or any infrastructure-as-code implementations involving reverse proxies in Kubernetes, Docker, or cloud-native environments. Examples: <example>Context: User needs help with nginx configuration. user: 'I need to set up nginx to proxy requests to multiple backend services' assistant: 'I'll use the reverse-proxy-architect agent to help you configure nginx for multi-backend proxying' <commentary>Since the user needs nginx reverse proxy configuration, use the Task tool to launch the reverse-proxy-architect agent.</commentary></example> <example>Context: User is troubleshooting proxy issues. user: 'My nginx is returning 502 errors when proxying to my Node.js app in Kubernetes' assistant: 'Let me use the reverse-proxy-architect agent to diagnose and fix your nginx 502 errors in Kubernetes' <commentary>The user has a reverse proxy issue in a Kubernetes environment, use the Task tool to launch the reverse-proxy-architect agent.</commentary></example> <example>Context: User needs load balancing setup. user: 'How do I configure Traefik for automatic SSL with Let's Encrypt in Docker Swarm?' assistant: 'I'll engage the reverse-proxy-architect agent to configure Traefik with automatic SSL in your Docker Swarm setup' <commentary>This involves reverse proxy configuration with Traefik, use the Task tool to launch the reverse-proxy-architect agent.</commentary></example>"
model: opus
color: purple
---
